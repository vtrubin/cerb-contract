...
{$page_context = 'cerberusweb.contexts.contract'}
{$page_context_id = $contract->id}
...
<div class="cerb-profile-toolbar">
	<form class="toolbar" action="{devblocks_url}{/devblocks_url}" onsubmit="return false;" style="margin-bottom:5px;">
		...
		<!-- Macros -->
		{devblocks_url assign=return_url full=true}c=profiles&type=contract&id={$page_context_id}-{$contract->name|devblocks_permalink}{/devblocks_url}
		{include file="devblocks:cerberusweb.core::internal/macros/display/button.tpl" context=$page_context context_id=$page_context_id macros=$macros return_url=$return_url}
		
		<!-- Edit -->
		<button type="button" id="btnDisplayContractEdit" title="{'common.edit'|devblocks_translate|capitalize}"><span class="glyphicons glyphicons-cogwheel"></span></button>
	</form>
...
<fieldset class="properties">
	<legend>{'Contract'|devblocks_translate|capitalize}</legend>
...
<script type="text/javascript">
	$(function() {
		...
		tabOptions.active = Devblocks.getjQueryUiTabSelected('profileContractTabs');
		
		var tabs = $("#profileContractTabs").tabs(tabOptions);
		
		$('#btnDisplayContractEdit').bind('click', function() {
			$popup = genericAjaxPopup('peek','c=internal&a=showPeekPopup&context={$page_context}&context_id={$page_context_id}',null,false,'550');
			$popup.one('contract_save', function(event) {
...
$(document).keypress(function(event) {
	...
	switch(event.which) {
		...
		case 58:  // (0) tab cycle
			try {
				idx = event.which-49;
				$tabs = $("#profileContractTabs").tabs();
				$tabs.tabs('option', 'active', idx);
			} catch(ex) { }
			break;
		case 101:  // (E) edit
			try {
				$('#btnDisplayContractEdit').click();
			} catch(ex) { }
			break;
		case 109:  // (M) macros
			try {
