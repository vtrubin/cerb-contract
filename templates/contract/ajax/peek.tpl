{$div_id = "peek{uniqid()}"}
{$peek_context = CerberusContexts::CONTEXT_CONTRACT}
<div id="{$div_id}">
    {$model->label}
...
<script type="text/javascript">
    $(function() {
        ...
        $popup.one('popup_open',function(event,ui) {
            $popup.dialog('option','title', "{'contracts.common.contract'|devblocks_translate|capitalize|escape:'javascript' nofilter}");
            ...
            $popup.find('.cerb-peek-profile').click(function(e) {
                if(e.shiftKey || e.metaKey) {
                    window.open('{devblocks_url}c=profiles&type=contract&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}', '_blank');

                } else {
                    document.location='{devblocks_url}c=profiles&type=contract&id={$dict->id}-{$dict->_label|devblocks_permalink}{/devblocks_url}';
                }
            });
